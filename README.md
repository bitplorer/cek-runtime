...
Publish (crates.io): tag `v*` or Actions → **publish-crates**. Trusted publishers must use environment `crates-io` (see workflow header). `cek-host-rust` is workspace **0.1.3**, unpublished until that workflow; peer hops already live at **0.1.3**.
