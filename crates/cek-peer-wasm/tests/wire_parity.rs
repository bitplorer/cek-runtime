//! Twin JSON wire: wasm + rust `ApplyRequest` / `ApplyResponse` field names
//! must stay equal. Hops own serde; this lock is not a shared JSON crate.

#[test]
fn apply_request_response_fields_match_rust_twin() {
    let wasm = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/lib.rs"));
    let rust = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../cek-peer-rust/src/lib.rs"
    ));
    ...
    assert_eq!(
        ...
        "wasm ApplyRequest public freeze"
    );
    assert_eq!(
        wasm_resp,
        vec!["receipt", "kv", "ui", "log"],
        "wasm ApplyResponse public freeze"
    );
