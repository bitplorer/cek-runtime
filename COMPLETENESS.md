...
Still not a published JSON Schema file dump (`cek-contract` types live in Rust). File-backed stores exist in the Rust kernel; Redis does not. Kernels are already on crates.io at workspace version **0.1.3**. What’s left to publish is the new hops `cek-peer-rust` and `cek-peer-pyo3` (Trusted Publishing / `publish-crates`).
